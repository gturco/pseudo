...
def split_fastas(bed):
   ...
   # works but should 50x mask ones have many scaffolds... double check
   f = bed.fasta.fasta_name
   fname = op.splitext(op.basename(f))[0]
   d = op.dirname(f) + "/{0}_split".format(fname)
   try: os.mkdir(d)
...
def main(qbed,sbed,missed_pairs, ncpu):
    ...
    #only bl2seq version is working cant get blastx to work
    #tblastx = "-outfmt 6 -matrix BLOSUM62 -word_size 3 -evalue 1e-10 -gap_open 11
    #-gapextend 1 -query {0} -query_loc {1}-{2} -subject {3} -subject_loc
    #{4}-{5}"
    tblastx = "/home/gturco/src/blast-2.2.25/bin/bl2seq -p tblastx -G 11 -E 1 -W 3 -e 1e-10 -D 1 -i {0} -j {1} -I {2},{3} -J {4},{5}"


    qfastas = get_mask_non_cds(qbed)
    sfastas = get_mask_non_cds(sbed)
    ...
    while any(pairs):
        ...
        def get_blastx_cmd(pair):
            """creates the dictionary values used to fill in tblastx cmd"""
            if pair is None: return None
            hit, gene = pair
            hstart, hstop = hit['start'], hit['end']
